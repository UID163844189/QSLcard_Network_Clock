...
#include <SPI.h>
//---------------修改此处""内的信息---------------------------------------------
const char ssid[] = "CU_Ve4J";                        // WiFi名
const char pass[] = "123456788";                      // WiFi密码
static const char ntpServerName[] = "ntp.aliyun.com"; // NTP服务器，上海交通大学
const int timeZone = 0;                               // 时区，北京时间为+8
//-----------------------------------------------------------------------------
WiFiUDP Udp;
...
void digitalClockDisplay()
{
    ...
    sendCommand(7, months % 10);
    sendCommand(6, days / 10);
    sendCommand(5, days % 10);

    sendCommand(4, minutes / 10);
    sendCommand(3, minutes % 10 + 128);
    sendCommand(2, seconds / 10);
    sendCommand(1, seconds % 10);

    delay(500);
    sendCommand(3, minutes % 10);
}
